<template>
  <el-card>
    <el-table :data="orderlist" v-loading="getOrdersLoading" stripe style="width: 100%">
      ...
      <el-table-column prop="m_name" label="商户" align="center"></el-table-column>
      ...
      <el-table-column prop="m_address" label="文件" align="center">
        <template v-slot="{row}">
          <div v-for="item in row.files" :key="item.index">
            <a :download="item.file_name" :href="item.file_url">{{item.file_name}}</a>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="order_status" label="订单状态" align="center">
        <template v-slot="{row}">
          ...
          <span v-else-if="row.order_status == 5" style="color: #708090;">已取消</span>
          <a v-if="row.order_status==3" href="#" @click="setStatus(row.id,4)">确认收货</a>
          <a v-if="row.order_status==1" href="#" @click="setStatus(row.id,2)">去支付</a>
        </template>
      </el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      delOrder(info) {
        //取消订单
        if(info.order_status === 1) this.setStatus(info.id,5)
        //删除订单
        if (info.order_status === 4 || info.order_status === 1) {
          this.$confirm('永久删除该订单, 是否继续?', '提示', {
            confirmButtonText: '继续',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.$put('/index/order/' + info.id, {
              'delete_for_user': 1
            }).then(
              res => {
                if (res.code !== 200) {
